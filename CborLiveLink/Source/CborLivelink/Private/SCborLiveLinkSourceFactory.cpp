...
#define LOCTEXT_NAMESPACE "CborLivelinkSourceEditor"

void SCborLivelinkSourceFactory::Construct(const FArguments& Args)
{
	...
	[
		...
		[
			...
			[
				...
				[
					SNew(STextBlock)
					.Text(LOCTEXT("CborPortNumber", "Port Number"))
				]
				+ SHorizontalBox::Slot()
				.HAlign(HAlign_Fill)
				.FillWidth(0.5f)
				[
					...
					.OnTextCommitted(this, &SCborLivelinkSourceFactory::OnEndpointChanged)
				]
			]
			+ SVerticalBox::Slot()
			.HAlign(HAlign_Right)
			...
			[
				SNew(SButton)
				.OnClicked(this, &SCborLivelinkSourceFactory::OnOkClicked)
				[
					SNew(STextBlock)
					.Text(LOCTEXT("Ok", "Ok"))
				]
...
void SCborLivelinkSourceFactory::OnEndpointChanged(const FText& NewValue, ETextCommit::Type)
{
...
FReply SCborLivelinkSourceFactory::OnOkClicked()
{
	TSharedPtr<SEditableTextBox> EditabledTextPin = EditabledText.Pin();
	if (EditabledTextPin.IsValid())
	{
